...
type DbService[DocType interface{}] interface {
    ...
    FindAllDocuments(ctx context.Context) (*mongo.Cursor, error)
}
...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
    ...
    if svc.DbName == "" {
        svc.DbName = enviro("AMBULANCE_API_MONGODB_DATABASE", "xpoky-ambulance-wl")
    }
...
func (m *mongoSvc[DocType]) FindAllDocuments(ctx context.Context) (*mongo.Cursor, error) {
    ctx, contextCancel := context.WithTimeout(ctx, m.Timeout)
    defer contextCancel()
    client, err := m.connect(ctx)
    if err != nil {
